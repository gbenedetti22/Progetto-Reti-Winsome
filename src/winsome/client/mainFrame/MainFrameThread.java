...
import winsome.client.UI.banners.PostBanner;
import winsome.client.UI.banners.UserBanner;
import winsome.client.UI.pages.*;
...
import static winsome.client.mainFrame.MainFrame.showErrorMessage;
import static winsome.client.mainFrame.MainFrame.showSuccessMessage;
...
public class MainFrameThread extends Thread {
    ...
    public MainFrameThread(MainFrame frame) throws IOException, NotBoundException {
        ...
        SocketChannel socket = SocketChannel.open(new InetSocketAddress((String) props.get(ClientProperties.NAMES.SERVER_ADDRESS), (int) props.get(ClientProperties.NAMES.DEFUALT_TCP_PORT)));
        socket.configureBlocking(true);

        Registry registry = LocateRegistry.getRegistry((String) props.get(ClientProperties.NAMES.RMI_ADDRESS), (int) props.get(ClientProperties.NAMES.RMI_FOLLOW_PORT));
    ...
    private void performGetLatestComments() {
        if (frame.getCurrentPage() instanceof PostPage page) {
            ...
            WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_COMMENTS_FROM_DATE, id, date);
    ...
    private void performRewin() {
        if (frame.getCurrentPage() instanceof HomePage) {
            ...
            WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.REWIN, id);
            serviceManager.submitRequest(request);
            WSResponse response = serviceManager.getResponse();
            if (response.code() != WSResponse.CODES.OK) {
                showErrorMessage(response.getBody());
    ...
    private void performLike() {
        if (frame.getCurrentPage() instanceof PostPage page) {
            String id = page.getId();
            WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.LIKE, id);
            serviceManager.submitRequest(request);
            WSResponse response = serviceManager.getResponse();
            if (response.code() != WSResponse.CODES.OK) {
                showErrorMessage(response.getBody());
    ...
    private void performDislike() {
        if (frame.getCurrentPage() instanceof PostPage page) {
            String id = page.getId();
            WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.DISLIKE, id);
            serviceManager.submitRequest(request);
            WSResponse response = serviceManager.getResponse();
            if (response.code() != WSResponse.CODES.OK) {
                showErrorMessage(response.getBody());
    ...
    private void performPublishComment() {
        ...
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.COMMENT, id, commentText);
        serviceManager.submitRequest(request);
        WSResponse response = serviceManager.getResponse();
        if (response.code() != WSResponse.CODES.OK) {
            showErrorMessage(response.getBody());
    ...
    private void performViewPost() {
        ...
        String id = banner.getID();
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_POST, id);
        serviceManager.submitRequest(request);
        WSResponse response = serviceManager.getResponse();
    ...
    private void performOpenRewin(PostBanner banner) {
        ...
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.OPEN_REWIN, author, id);
        serviceManager.submitRequest(request);
        WSResponse response = serviceManager.getResponse();
    ...
    private void performDeletePost() {
        if (frame.getCurrentPage() instanceof ProfilePage profile) {
            ...
            if (clickedBanner.isRewin()) {
                request = new WSRequest(WSRequest.WS_OPERATIONS.REMOVE_REWIN, id);
            } else {
                request = new WSRequest(WSRequest.WS_OPERATIONS.REMOVE_POST, id);
            }
    ...
    private void performFollow() {
        if (frame.getCurrentPage() instanceof DiscoverPage) {
            ...
            serviceManager.submitRequest(new WSRequest(WSRequest.WS_OPERATIONS.FOLLOW, username));
            WSResponse response = serviceManager.getResponse();
            if (response.code() != WSResponse.CODES.OK) {
                showErrorMessage(response.getBody());
                return;
    ...
    private void performUnfollow() {
        if (frame.getCurrentPage() instanceof DiscoverPage) {
            ...
            serviceManager.submitRequest(new WSRequest(WSRequest.WS_OPERATIONS.UNFOLLOW, username));
            WSResponse response = serviceManager.getResponse();
            if (response.code() != WSResponse.CODES.OK) {
                showErrorMessage(response.getBody());
                return;
    ...
    private void performDiscover() {
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_FRIENDS_BY_TAG);
    ...
    private void performPublishPost() {
        if (frame.getCurrentPage() instanceof ProfilePage page) {
            ...
            WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.CREATE_POST, title, content);
            serviceManager.submitRequest(request);
    ...
    private void performLogout() {
        ...
        storage.clear();
        Pages.resetPages();
        serviceManager.submitRequest(new WSRequest(WSRequest.WS_OPERATIONS.LOGOUT));
        WSResponse response = serviceManager.getResponse();
        if (response.code() != WSResponse.CODES.OK) {
    ...
    private void downloadTransactions() {
        WSRequest transactionsRequest = new WSRequest(WSRequest.WS_OPERATIONS.GET_TRANSACTIONS);
    ...
    private void setMyFollowing() {
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_FOLLOWING);
        serviceManager.submitRequest(request);
        WSResponse response = serviceManager.getResponse();
    ...
    private void downloadPosts() {
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_FRIENDS_POSTS);
        serviceManager.submitRequest(request, response -> {
    ...
    private void downloadMyPosts() {
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_MY_POSTS);
        serviceManager.submitRequest(request, response -> {
            Type type = new TypeToken<ArrayList<SimplePost>>() {
            }.getType();
            ArrayList<SimplePost> posts = gson.fromJson(response.getBody(), type);
    ...
    private void downloadLatestPosts() {
        ...
        WSRequest request = new WSRequest(WSRequest.WS_OPERATIONS.GET_FRIENDS_POST_FROM_DATE, gson.toJson(dateMap));
